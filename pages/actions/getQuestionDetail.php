<?php
	...
	$sql='SELECT id, title, description, created_time, preferred_time,stdnt_user_id, stdnt_first_name, stdnt_last_name,status, num_liked, concern FROM t_question WHERE id = '.$questionId.' LIMIT 1';
	$result=$mysqli->query($sql);
	if($result) {
		$question=array();
		if($row = $result->fetch_assoc()){
